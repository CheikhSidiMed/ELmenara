...
$due_amount = isset($_GET['due_amount']) ? $_GET['due_amount'] : '';
$garant_id = isset($_GET['garant_id']) ? $_GET['garant_id'] : '';
...
if (!empty($receipt_id)) {
    ...
    $sql_1 = "SELECT balance FROM garants WHERE id = ?";
    $stmt_1 = $conn->prepare($sql_1);
    $stmt_1->bind_param('i', $garant_id);
    $stmt_1->execute();
...
<body>
    <div class="container my-5">
        ...
        <div class="summary-container">
            ...
            <div>
                <span> رصيد حساب الكافل(ة)</span> : <?php echo $balance; ?>
            </div>
            <div>
                <span>المبلغ المدفوع</span> : <?php echo $total_paid_sum; ?>
            </div>
            <div class="">
                <span>المبلغ المتبقي</span> : <?php echo $due_amount; ?>
            </div>
        </div>

        <!-- Print Button -->
        <div class="text-center mt-4">
            <button class="btn btn-success print-button px-5" onclick="window.print()">طباعة</button>
        </div>
    </div>
</div>
</body>
