...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (!empty($student_activity_id) && !empty($paid_amount) && !empty($payment_method)) {
        ...
        $stmt = $conn->prepare("
            SELECT sa.id, a.id AS activitie_id, a.activity_name AS activitie_name, s.name AS student_name 
        ...
        $stmt->execute();
        $stmt->bind_result($student_activity_id, $activitie_id, $activitie_name, $student_name);
        $stmt->fetch();
        $stmt->close();
        ...
        $stmt = $conn->prepare("
            INSERT INTO activities_payments (student_ac_id_etrang, payment_date, paid_amount, payment_method, bank_id, activitie_id)
            VALUES (?, ?, ?, ?, ?, ?)
        ");
        ...
        $stmt->bind_param("isssss", $student_activity_id, $payment_date, $paid_amount, $payment_method, $bank_id, $act_id);
        ...
        if ($stmt->execute()) {
            ...
            $stmt = $conn->prepare("
                ...
                VALUES (NOW(), ?, ?, ?)
            ");
            $stmt->bind_param("sss", $paid_amount, $transaction_description, $user_id);
            $stmt->execute();
            $receipts_id = $conn->insert_id;
                ...
                $stmt = $conn->prepare("
                INSERT INTO combined_transactions (type, etrg_id, description, paid_amount, payment_method, bank_id, user_id, fund_id)
                VALUES ('plus', ?, ?, ?, ?, ?, ?, ?)
                ");

                $stmt->bind_param("sssssss", $student_activity_id, $transaction_description, $paid_amount, $payment_method, $bank_id, $user_id, $fund_id);
                $stmt->execute();
            ...
            $stmt = $conn->prepare("
                INSERT INTO transactions (transaction_description, amount, transaction_type, tran_etrang_id, activitie_id, fund_id, bank_account_id, user_id)
                VALUES (?, ?, 'plus', ?, ?, ?, ?, ?)
            ");
            ...
            $stmt->bind_param("siisiii", $transaction_description, $paid_amount, $student_activity_id, $activitie_id, $fund_id, $bank_account_id, $user_id);
