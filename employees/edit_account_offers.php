...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!empty($account_id) && !empty($account_name) && !empty($category) && is_numeric($account_balance)) {
        ...
        if ($stmt->execute()) {
            ...
            header('Location: manage_khadamat.php?message=Account updated successfully');
            exit;
        } else {
            // Redirect back with error message
            header('Location: manage_khadamat.php?error=Failed to update account');
            exit;
        }
    } else {
        // Redirect back with validation error
        header('Location: manage_khadamat.php?error=Invalid input data');
        exit;
    }
}
