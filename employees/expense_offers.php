...
while ($row = $result->fetch_assoc()) {
    $accounts[] = $row;
    $total_amount += $row['account_balance'];
}
...
<body>
    ...
    <div class="container main-container">
        <div class="row align-items-center">
            ...
            <div class="col-10">
                <h2 class="header-title">حسابات الغير</h2>
            </div>
        </div>
         <!-- Home Button -->
    ...
    <?php foreach ($accounts as $account) : ?>
        <tr>
            <td>
                <a href="offers_preview.php?account_number=<?= htmlspecialchars($account['id']) ?>" class="view-icon">&#128065;</a>
            </td>
            <td><?= htmlspecialchars($account['account_name']) ?></td>
            <td><?= htmlspecialchars($account['category']) ?></td>
            <td >
                <span style="font-weight: bold; font-size: 16px">
                    <?= htmlspecialchars(number_format($account['account_balance'])) ?>
                </span>
            </td>
        </tr>
    ...
    <script>
        document.getElementById('search').addEventListener('input', function() {
            ...
            fetch('fetch_expense_accounts.php?search=' + encodeURIComponent(searchQuery))
